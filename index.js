...
const config = {
    A187: '🌀VLYZANBOT🌀',
    instagram: 'https://instagram.com/fdy_moonza.ig',
    nomer: 'wa.me/6282317380554',
    aktif: '24 JAM',
    youtube: 'https://youtube.com/channel/UCEPzMNcqhU6jaIPHj5MCHHg',
...
client.on('qr', qr => {
   ...
   console.log(`[ ${time} ] QR code Telah Jalan, subscribe SyaFa DetSon`)
})
...
// client.on('user-presence-update', json => console.log(json.id + ' presence is => ' + json.type)) || console.log(`${time}: Bot by ig:@fdy_moonza`)

client.on('message-status-update', json => {
   const participant = json.participant ? ' (' + json.participant + ')' : ''
   console.log(`[ ${time} ] => bot by ig:@fdy_moonza.ig`)
})
   ...
   case 'fb':
           fb(value)
               .then(data => {
                   ...
                   let hasil = `Pilih resolusi nya ya \n\n\n HD ${resultHD} \n\n\n SD ${resultSD}`
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
 ...
 case 'infogempa':
   ...
   case 'chord':
           chord(value)
               .then(data => {
                   const { result } = data
                   let hasil = `Nih chord lagu *${value}* sudah siap\n\n  _${result}_ `
                   client.sendMessage(id, hasil, MessageType.text)
               })
               .catch(err => {
                   console.log(err)
